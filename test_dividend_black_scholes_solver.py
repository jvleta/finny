...
class TestDividendBlackScholesConfig(unittest.TestCase):
    ...
    def test_continuous_dividend_initialization(self):
        ...
        config = DividendBlackScholesConfig(dividend_yield=0.03, option_type="put")
    ...
    def test_discrete_dividend_initialization(self):
        ...
        config = DividendBlackScholesConfig(T=1.0, discrete_dividends=dividends)
    ...
    def test_dividend_after_expiry_raises_error(self):
        ...
        dividends = [DividendEvent(time=1.5, amount=1.0)]  # After T=1.0

        with self.assertRaises(ValueError):
            DividendBlackScholesConfig(T=1.0, discrete_dividends=dividends)
...
class TestBoundaryConditionsDividend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = DividendBlackScholesConfig(
            S_max=200.0, K=100.0, T=1.0, r=0.05, sigma=0.2, dividend_yield=0.02
        )
    ...
    def test_call_boundary_conditions(self):
        ...
        # At t=T (expiry)
        lower_bc, upper_bc = boundary_conditions_dividend(
            self.S, self.config.T, self.config
        )
        self.assertEqual(lower_bc, 0.0)
    ...
    def test_discrete_dividend_boundary_adjustment(self):
        ...
        lower_bc_after, upper_bc_after = boundary_conditions_dividend(
            self.S, 0.7, config
        )
...
class TestAnalyticalDividendBlackScholes(unittest.TestCase):
    ...
    def test_continuous_dividend_call(self):
        ...
        result = analytical_dividend_black_scholes(S, K, T, r, sigma, q, None, "call")
    ...
    def test_continuous_dividend_put(self):
        ...
        result = analytical_dividend_black_scholes(S, K, T, r, sigma, q, None, "put")
...
class TestDividendCrankNicolsonSolver(unittest.TestCase):
    ...
    def test_initial_condition(self):
        ...
        expected_payoff = payoff_function_dividend(
            S_grid, self.config.K, self.config.option_type
        )
    ...
    def test_convergence_to_analytical_continuous_dividend(self):
        ...
        with warnings.catch_warnings():
            ...
            S_grid, t_grid, V_grid = dividend_crank_nicolson_solver(config, N_S, N_t)[
                :3
            ]
    ...
    def test_dividend_yield_impact(self):
        ...
        with warnings.catch_warnings():
            ...
            _, _, V_with_div = dividend_crank_nicolson_solver(
                config_with_div, N_S, N_t
            )[:3]
    ...
    def test_plot_dividend_results_execution(self):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                plot_dividend_results(
                    self.S_grid, self.t_grid, self.V_grid, self.config, self.div_indices
                )
...
class TestDividendEdgeCases(unittest.TestCase):
    ...
    def test_zero_dividend_yield(self):
        ...
        config = DividendBlackScholesConfig(dividend_yield=0.0)
    ...
    def test_high_dividend_yield(self):
        ...
        config = DividendBlackScholesConfig(dividend_yield=0.15)  # 15% dividend yield

        with warnings.catch_warnings():
            warnings.simplefilter("ignore")

            try:
                S_grid, t_grid, V_grid = dividend_crank_nicolson_solver(
                    config, N_S=20, N_t=30
                )[:3]
    ...
    def test_large_discrete_dividend(self):
        ...
        config = DividendBlackScholesConfig(
            S_max=100.0, K=50.0, discrete_dividends=dividends
        )
