...
class TestBlackScholesConfig(unittest.TestCase):
    ...
    def test_custom_initialization(self):
        ...
        config = BlackScholesConfig(
            S_max=150.0, K=110.0, T=0.5, r=0.03, sigma=0.25, option_type="put"
        )
...
class TestBoundaryConditions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = BlackScholesConfig(S_max=200.0, K=100.0, T=1.0, r=0.05, sigma=0.2)
    ...
    def test_call_boundary_conditions(self):
        ...
        expected_upper = self.config.S_max - self.config.K * np.exp(
            -self.config.r * self.config.T
        )
...
class TestCrankNicolsonSolver(unittest.TestCase):
    ...
    def test_initial_condition(self):
        ...
        expected_payoff = payoff_function(
    ...
    def test_convergence_to_analytical(self):
        ...
        for S_test in test_prices:
            ...
            self.assertLess(
                relative_error, 0.15
            )  # Less than 15% error (finite difference approximation)
